    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6">
        ...
        <button
          ...
          disabled={uploading}
          className="w-full border-2 border-dashed border-gray-300 rounded-xl p-10 text-center cursor-pointer hover:border-bs-accent-light hover:bg-blue-50/30 transition-all disabled:opacity-50"
        >
          {uploading ? (
            <Loader2 size={36} className="mx-auto text-bs-accent animate-spin" />
          ) : (
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6">
        ...
        <div className="mt-4">
          <label className="block text-sm font-medium text-gray-700 mb-1">Ring-Geschwindigkeit (°/Sek.)</label>
          <div className="flex items-center gap-3">
            <input type="range" min="1" max="30" step="1" value={config.ringSpeed || 8}
              onChange={(e) => setConfig(prev => ({ ...prev, ringSpeed: parseFloat(e.target.value) || 8 }))}
              className="flex-1" />
            <span className="text-sm font-mono text-gray-600 w-12 text-center">{config.ringSpeed || 8}°/s</span>
          </div>
          <p className="text-xs text-gray-400 mt-1">Drehgeschwindigkeit des Kategorie-Rings auf der Startseite</p>
        </div>
        <div className="mt-4">
          <label className="block text-sm font-medium text-gray-700 mb-1">Erd-Rotation (px/Sek.)</label>
          <div className="flex items-center gap-3">
            <input type="range" min="5" max="80" step="1" value={config.earthSpeed || 20}
              onChange={(e) => setConfig(prev => ({ ...prev, earthSpeed: parseFloat(e.target.value) || 20 }))}
              className="flex-1" />
              ...
              <div key={slide.id} className={`flex items-center gap-4 p-3 rounded-lg border ${slide.active ? "bg-white border-gray-200" : "bg-gray-50 border-gray-100 opacity-50"}`}>
                ...
                <div className="flex items-center gap-1">
                  ...
                  <button onClick={() => handleDelete(slide.id)}
                    className="p-1.5 text-gray-400 hover:text-bs-accent"><Trash2 size={14} /></button>
                </div>
              </div>
            ))}
          </div>
